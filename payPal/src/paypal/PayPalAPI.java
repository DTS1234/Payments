...
public class PayPalAPI {
	...
	public String logIn(String username, String password) {
		if(checkIfUserAndPasswordMatch(username, password)) {
	...
	private String searchUserName(String userName) {
		...
		for (int i = 0; i < users.length; i++) {
			if (userName.equals(users[i][0])) {
				return (String) users[i][0];
			}
		}
		return null;
	}

	private String searchPassword(String password) {

		for (int i = 0; i < users.length; i++) {
			if (password.equals(users[i][1])) {
				return (String) users[i][1];
			}
		}
		return null;
	}

	private boolean checkIfUserAndPasswordMatch(String userName, String password) {

		String userNameToMatch = searchUserName(userName);
		String passwordToMatch = searchPassword(password);

		if (userNameToMatch == null || passwordToMatch == null)
			return false;
		else {
			for (int i = 0; i < users.length; i++) {
				if (users[i][0].equals(userNameToMatch))
					return users[i][1].equals(passwordToMatch);
			}
		}
	 ...
	 * 
	 * @param token         the previously generated session token that should have
	 *                      been returned to the client during the login process
	...
	public boolean checkout(String token, String transactionId, double amount) {
		...
		if(tokenList.contains(token) && amount >= 0.01)
			return true;
